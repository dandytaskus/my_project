  ...
  app.controller('UpdateTask',function($scope,$http, $window){
    ...
    $scope.attachments=[];
      ...
      $scope.add_email=function(){
        ...
        $scope.email_recieve = $scope.email_recieve.replace(";"," ");
        $scope.email_recieve = $scope.email_recieve.replace(","," ");

        if ($scope.email_recieve.indexOf(" ") >= 0){ //=======If Space are detected
          var array_email=$scope.email_recieve.split(" ");
          console.log(array_email);
          angular.forEach(array_email,function(email, index){
             
             if(ValidateEmail(email)){
                  $scope.email_recievers.push(email);
             }

          });
          $scope.email_recieve="";
          //alert("space are here!");
        }else{                                       //=======Else individual email inserted
             if(ValidateEmail($scope.email_recieve)){
                $scope.email_recievers.push($scope.email_recieve);
                $scope.email_recieve=""; 
